...
class Pedido extends CI_Controller {
	...
	public function index($opc_id){
		...
		if($_POST){
			...
			$cns_pedidos=$this->pedido_model->lista_pedidos_buscador($text,$f1,$f2,$rst_cja->emp_id,$txt_est,$rst_cja->emi_id);
		}else{
			$text= '';
			$f1= date('Y-m-d');
			$f2= date('Y-m-d');
			$est="";
			$cns_pedidos=$this->pedido_model->lista_pedidos_buscador($text,$f1,$f2,$rst_cja->emp_id,$est,$rst_cja->emi_id);
		}
		...
		if($this->form_validation->run()){
		    ...
		    if(!empty($ped_id)){
				...
				//redirect(base_url().strtolower($rst_opc->opc_direccion).$opc_id);
				 ...
				 redirect(base_url().'pedido/show_ptalon/'. $ped_id.'/'.$opc_id);
    ...
    public function show_ptalon($id,$opc_id){
    	$permisos=$this->backend_model->get_permisos($opc_id,$this->session->userdata('s_rol'));
    	$rst_opc=$this->opcion_model->lista_una_opcion($opc_id);
    	$etiqueta='Pedido.pdf';
    	if($permisos->rop_reporte){
    		$data=array(
					'titulo'=>'Pedidos',
					'regresar'=>strtolower($rst_opc->opc_direccion).$rst_opc->opc_id,
					'direccion'=>"pedido/show_talon/$opc_id/$id",
				);
			$this->load->view('layout/header',$this->menus());
			$this->load->view('layout/menu',$this->menus());
			$this->load->view('pdf/frame',$data);
    ...
    public function show_talon($opc_id,$id){
    		$permisos=$this->backend_model->get_permisos($opc_id,$this->session->userdata('s_rol'));
			$rst_opc=$this->opcion_model->lista_una_opcion($opc_id);
		...
		//pagos pedido
    		...
    		$this->html2pdf->filename('talon_pedido.pdf');
			$this->html2pdf->paper(array(0,0,204.75,533.66), 'portrait');
			
    		$this->html2pdf->html(utf8_decode($this->load->view('pdf/pdf_talon_pedido', $data, true)));
    		$this->html2pdf->output(array("Attachment" => 0));
    		//$this->load->view('pdf/pdf_talon_pedido', $data);
